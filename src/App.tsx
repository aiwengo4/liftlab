  ...
  return (
    <main className={`page ${groupedPreview ? 'grouped-layout' : 'legacy-layout'}`}>
      ...
      <form onSubmit={(event) => { event.preventDefault(); run() }} noValidate>
        <details className="section-card top-disclosure">
          ...
          {groupedPreview ? <EmployeeFloorColumns form={form} update={update} /> : <div className="floor-table" aria-label="Настройки этажей">
            ...
            {[...form.floors].reverse().map((floorValue) => <div className="floor-row" key={floorValue.floor}>
              ...
              <input aria-label={`Сотрудники на этаже ${floorValue.floor}`} type="number" min="0" step="1" value={floorValue.employees} aria-invalid={!Number.isSafeInteger(floorValue.employees) || floorValue.employees < 0 || floorValue.employees > 10000} disabled={form.distributionMode === 'equal'} onChange={(event) => update({ floors: form.floors.map((floor) => floor.floor === floorValue.floor ? { ...floor, employees: Number(event.target.value) } : floor) })} />
...
function EmployeeFloorColumns({ form, update }: { form: ScenarioFormState; update: (patch: Partial<ScenarioFormState>) => void }) {
  ...
  return <div className="floor-columns" aria-label="Численность сотрудников по этажам">{columns.map((floors, columnIndex) => <div className="floor-column" key={columnIndex}><strong className="floor-column-title">Этажи {floors[0].floor}–{floors[floors.length - 1].floor}</strong>{floors.map((floorValue) => <label className="floor-count-row" key={floorValue.floor}><span>{floorValue.floor} этаж</span><div className="input-with-suffix"><input aria-label={`Сотрудники на этаже ${floorValue.floor}`} type="number" min="0" step="1" value={floorValue.employees} aria-invalid={!Number.isSafeInteger(floorValue.employees) || floorValue.employees < 0 || floorValue.employees > 10000} disabled={form.distributionMode === 'equal'} onChange={(event) => update({ floors: form.floors.map((floor) => floor.floor === floorValue.floor ? { ...floor, employees: Number(event.target.value) } : floor) })} /><em>чел.</em></div></label>)}</div>)}</div>
}
...
function StairSettings({ form, errors, submitted, update }: SettingsBlockProps) {
  return <div className="grouped-settings"><label className="check-line"><input type="checkbox" checked={form.stairsConvenient} onChange={(event) => update({ stairsConvenient: event.target.checked })} />Лестницей удобно пользоваться</label><p className="hint">Для обычных перемещений вероятности удобной и неудобной лестницы задаются отдельно. При перегрузе на обеде значения для неудобной лестницы рассчитываются с помощью коэффициента.</p><div className="field-grid two"><NumberField label="Максимальный обычный путь" suffix="эт." value={form.maxVoluntaryStairFloors} min={1} max={6} error={submitted ? errors.maxVoluntaryStairFloors : undefined} onChange={(maxVoluntaryStairFloors) => update({ maxVoluntaryStairFloors })} /><NumberField label="Коэффициент для неудобной лестницы" value={form.inconvenientStairFactor} min={0} max={1} step={0.1} error={submitted ? errors.inconvenientStairFactor : undefined} onChange={(inconvenientStairFactor) => update({ inconvenientStairFactor })} hint="Применяется только при перегрузе на обеде. Значение 0,5 уменьшает вероятности вдвое." /></div><details className="nested-disclosure"><summary>Обычные дневные перемещения</summary><DistributionEditor legend="Удобная лестница" labels={['1 этаж','2 этажа','3 этажа','4 этажа','5 этажей','6 этажей']} values={form.convenientStairProbabilities} disabledAfter={form.maxVoluntaryStairFloors} error={errors.convenientStairProbabilities} requireTotal={false} onChange={(convenientStairProbabilities) => update({ convenientStairProbabilities })} /><DistributionEditor legend="Неудобная лестница" labels={['1 этаж','2 этажа','3 этажа','4 этажа','5 этажей','6 этажей']} values={form.inconvenientStairProbabilities} disabledAfter={form.maxVoluntaryStairFloors} error={errors.inconvenientStairProbabilities} requireTotal={false} onChange={(inconvenientStairProbabilities) => update({ inconvenientStairProbabilities })} /></details><details className="nested-disclosure"><summary>Спуск на обед при перегрузе</summary><label className="check-line"><input type="checkbox" checked={form.lunchOverloadStairsEnabled} onChange={(event) => update({ lunchOverloadStairsEnabled: event.target.checked })} />Разрешить спуск по лестнице при перегрузе</label><p className="hint">Очередь считается перегруженной, если перед сотрудником ждут не менее чем 1,5 вместимости одного лифта. Решение принимается один раз — только по пути в столовую и только при движении вниз.</p><DistributionEditor legend="Вероятность для удобной лестницы" labels={['1 этаж','2 этажа','3 этажа','4 этажа','5 этажей','6 этажей','7+ этажей']} values={form.lunchOverloadStairProbabilities} error={errors.lunchOverloadStairProbabilities} requireTotal={false} minPercent={5} onChange={(lunchOverloadStairProbabilities) => update({ lunchOverloadStairProbabilities })} /><p className="hint">Для неудобной лестницы значения умножаются на {form.inconvenientStairFactor}. Например, 5% при коэффициенте 0,5 превращаются в 2,5%.</p></details><details className="nested-disclosure"><summary>Кнопки вызова</summary><p className="hint">Базовая модель использует отдельные кнопки «вверх» и «вниз».</p></details></div>
}
...
function NumberField({ label, suffix, value, min, max, step = 1, error, hint, onChange }: { label: string; suffix?: string; value: number; min: number; max: number; step?: number; error?: string; hint?: string; onChange: (value: number) => void }) {
  ...
  return <label className="field" htmlFor={id}><span>{label}</span><div className="input-with-suffix"><input id={id} type="number" min={min} max={max} step={step} value={value} aria-invalid={Boolean(visibleError)} aria-describedby={hint || visibleError ? descriptionId : undefined} onChange={(event) => onChange(Number(event.target.value))} />{suffix && <em>{suffix}</em>}</div>{hint && <small id={descriptionId} className="field-hint">{hint}</small>}{visibleError && <small id={descriptionId} className="field-error" role="alert">{visibleError}</small>}</label>
}
...
function DistributionEditor({ legend, labels, values, onChange, error, disabledAfter, requireTotal = true, minPercent = 0 }: { legend: string; labels: readonly string[]; values: readonly number[]; onChange: (values: number[]) => void; error?: string; disabledAfter?: number; requireTotal?: boolean; minPercent?: number }) {
  ...
  return <fieldset className="compact-fieldset"><legend>{legend}</legend><div className="distribution-grid">{labels.map((label,index) => { const percent = values[index] * 100; const invalid = !Number.isFinite(percent) || percent < minPercent || percent > 100; return <label key={label}><span>{label}</span><div className="input-with-suffix"><input type="number" min={minPercent} max="100" step="0.1" value={percent} aria-invalid={invalid} disabled={disabledAfter !== undefined && index >= disabledAfter} onChange={(event) => onChange(values.map((value,current) => current === index ? Number(event.target.value) / 100 : value))} /><em>%</em></div>{disabledAfter !== undefined && index >= disabledAfter && <small>не используется</small>}</label> })}</div>{requireTotal && <div className={`distribution-total ${error ? 'has-error' : ''}`} aria-live="polite">Сумма: {Number(total.toFixed(1))}%</div>}{error && <p className="field-error">{error}</p>}</fieldset>
}
