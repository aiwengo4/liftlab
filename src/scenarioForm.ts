...
export function validateScenarioForm(values: ScenarioFormState): ScenarioFormErrors {
  ...
  if (values.lunchOverloadStairProbabilities.length !== 7) errors.lunchOverloadStairProbabilities = 'Укажите семь значений: для 1–6 этажей и для 7 этажей и более'
  else if (values.lunchOverloadStairProbabilities.some((value) => !Number.isFinite(value) || value < 0 || (value > 0 && value < 0.05) || value > 1)) errors.lunchOverloadStairProbabilities = 'Укажите 0% для отключения или значение от 5 до 100%'
