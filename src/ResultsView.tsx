...
import type { MetricSlice, RouteMetricGroup, TimeStatistics } from './engine/metrics'
...
export function ResultsView({ result, form }: { result: BrowserSimulationResult; form: ScenarioFormState }) {
  ...
  const [hourlyElevatorOnly, setHourlyElevatorOnly] = useState(false)
  ...
  const hours = result.metrics.hours.filter((hour) => hourlyElevatorOnly ? hour.routeGroups.elevator.routeCount > 0 : hour.routeCount > 0)
  ...
  return <section className="results" aria-labelledby="results-title">
    ...
    <div className="metric-grid result-summary"><Metric label="Среднее ожидание" value={duration(result.metrics.wholeDay.waiting.meanSeconds)} /><Metric label="Медиана ожидания" value={duration(result.metrics.wholeDay.waiting.medianSeconds)} /><Metric label={<><PercentileLabel value={90} /> ожидания</>} value={duration(result.metrics.wholeDay.waiting.p90Seconds)} /><Metric label="Средний полный маршрут" value={duration(result.metrics.wholeDay.total.meanSeconds)} /><Metric label="Медиана полного маршрута" value={duration(result.metrics.wholeDay.total.medianSeconds)} /><Metric label="Максимальная очередь" value={result.operationalMetrics.queue.maximumWaitingInBuilding + ' чел.'} /></div>
    ...
    <section className="result-block"><header><h3>Время маршрутов</h3><p>Маршрут относится к периоду по фактическому времени начала.</p></header>
      ...
      <div className="route-statistics-groups">
        <RouteStatistics title="Маршруты с лифтом" note="Включая маршруты, где после лифта пришлось пройти часть пути по лестнице." group={slice.routeGroups.elevator} allRoutes={slice.routeCount} rows={['waiting', 'riding', 'total']} />
        <RouteStatistics title="Маршруты только по лестнице" note="Сотрудник не пользовался лифтом на этом маршруте." group={slice.routeGroups.stairs} allRoutes={slice.routeCount} rows={['total']} />
        <RouteStatistics title="Все маршруты" note="Итог по всем способам передвижения." group={slice.routeGroups.all} allRoutes={slice.routeCount} rows={['total']} />
      </div>
      <p className="result-note">Для каждой группы показаны количество маршрутов и доля от всех маршрутов выбранного периода. «Весь день» может включать время вне трёх периодов.</p>
    </section>
    ...
    <details className="result-disclosure"><summary><span>По часам</span><small>{hours.length} часов с маршрутами</small></summary><div className="result-details"><div className="hourly-route-filter" role="group" aria-label="Какие маршруты показывать по часам"><button type="button" aria-pressed={!hourlyElevatorOnly} onClick={() => setHourlyElevatorOnly(false)}>Все маршруты</button><button type="button" aria-pressed={hourlyElevatorOnly} onClick={() => setHourlyElevatorOnly(true)}>Только с лифтом</button></div><p className="result-note">{hourlyElevatorOnly ? `Исключено маршрутов только по лестнице: ${result.metrics.wholeDay.routeGroups.stairs.routeCount}. Поэтому пересчитываются количество и показатели «Полное»; ожидание и поездка всегда относятся только к лифту.` : `Полное время включает маршруты с лифтом и ${result.metrics.wholeDay.routeGroups.stairs.routeCount} маршрутов только по лестнице. Ожидание и поездка в обоих режимах считаются только для лифта.`}</p><p className="table-swipe hourly-table-hint"><span aria-hidden="true">←</span> В таблице есть дополнительные показатели — прокрутите её вбок <span aria-hidden="true">→</span></p><TableScroll><table className="result-table"><caption className="sr-only">Почасовые показатели</caption><thead><tr><th>Час</th><th>Маршруты</th><th>Ждали: сред.</th><th>Ждали: медиана</th><th>Ждали: <PercentileLabel value={90} /></th><th>Ехали: сред.</th><th>Ехали: медиана</th><th>Ехали: <PercentileLabel value={90} /></th><th>Полное: сред.</th><th>Полное: медиана</th><th>Полное: <PercentileLabel value={90} /></th></tr></thead><tbody>{hours.map((hour) => { const hourly = hourlyElevatorOnly ? hour.routeGroups.elevator : hour; return <tr key={hour.hour}><th scope="row">{String(hour.hour).padStart(2,'0')}:00</th><td>{hourly.routeCount}</td><Cells stats={hourly.waiting} /><Cells stats={hourly.riding} /><Cells stats={hourly.total} /></tr> })}</tbody></table></TableScroll></div></details>
...
type StatisticKey = 'waiting' | 'riding' | 'total'
const statisticLabels: Record<StatisticKey, string> = { waiting: 'Ждал лифт', riding: 'Ехал в лифте', total: 'Полный маршрут' }
function RouteStatistics({ title, note, group, allRoutes, rows }: { title: string; note: string; group: RouteMetricGroup; allRoutes: number; rows: readonly StatisticKey[] }) { return <section className="route-statistics"><header><div><h4>{title}</h4><p>{note}</p></div><strong>{countShare(group.routeCount, allRoutes)}</strong></header><TableScroll><table className="result-table"><caption className="sr-only">{title}: статистика времени</caption><thead><tr><th>Показатель</th><th>Среднее</th><th>Медиана</th><th><PercentileLabel value={75} /></th><th><PercentileLabel value={90} /></th><th><PercentileLabel value={95} /></th><th>Маршруты</th></tr></thead><tbody>{rows.map((key) => { const item: TimeStatistics = group[key]; return <tr key={key}><th scope="row">{statisticLabels[key]}</th><td>{duration(item.meanSeconds)}</td><td>{duration(item.medianSeconds)}</td><td>{duration(item.p75Seconds)}</td><td>{duration(item.p90Seconds)}</td><td>{duration(item.p95Seconds)}</td><td>{countShare(item.sampleSize, allRoutes)}</td></tr> })}</tbody></table></TableScroll></section> }
...
function Histogram({ title, values }: { title:string; values:readonly number[] }) { const bins=buildHistogram(values); const max=Math.max(1,...bins.map((item)=>item.count)); return <figure className="histogram"><figcaption><strong>{title}</strong><span>{values.length} маршрутов</span></figcaption>{bins.length===0?<p>Нет маршрутов</p>:<><div className="histogram-bars" aria-hidden="true">{bins.map((item,index)=><div className="histogram-column" key={index}><b>{item.count}</b><i style={{height:Math.max(3,item.count/max*100)+'%'}}/><small>{histogramLabel(item)}</small></div>)}</div><ol className="sr-only">{bins.map((item,index)=><li key={index}>{histogramLabel(item)}: {item.count}</li>)}</ol></>}</figure> }
...
function Metric({label,value,note}:{label:React.ReactNode;value:string;note?:string}) { return <div className="metric"><span>{label}</span><strong>{value}</strong>{note&&<small>{note}</small>}</div> }
function PercentileLabel({ value }: { value: 75 | 90 | 95 }) { return <span className="percentile-label">P{value}<button type="button" className="help-tooltip" aria-label={`Что показывает P${value}?`}>?<span role="tooltip">P{value} — время, в которое уложились {value}% маршрутов. Остальные {100 - value}% заняли больше времени.</span></button></span> }
